...
# 201-dim feature layout: (lm.x, lm.y, vis/1.0) × 67 landmarks  — 정규화 좌표 (0~1)
...
def augment_spatial(arr: np.ndarray,
    ...
    """x·y 정규화 좌표(0~1)에 이동·스케일·노이즈 적용. visibility/confidence 열은 건드리지 않음.

    max_shift  : 상하좌우 최대 이동량 (정규화 좌표, 0.03 = 3%)
    scale_range: 스케일 변화 비율 (±5%, 신체 중심 기준 줌인·아웃)
    noise_std  : 랜드마크 검출 오차 시뮬레이션 (정규화 좌표, 0.003 ≈ 2px/640px)
    """
    arr = arr.copy()
